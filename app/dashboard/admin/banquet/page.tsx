    ...
    if (files.length > 0) {
      ...
      setFormState((prev: BanquetFormState) => ({
        ...
        [`upload${galleryType.charAt(0).toUpperCase() + galleryType.slice(1)}`]: [
          ...(prev[`upload${galleryType.charAt(0).toUpperCase() + galleryType.slice(1)}` as keyof BanquetFormState] as File[] || []),
          ...files
        ],
        [`${galleryType}Preview`]: [
          ...(prev[`${galleryType}Preview` as keyof BanquetFormState] as string[] || []),
          ...previewUrls
        ],
      }));
                          ...
                          <DropdownMenuItem
                            onSelect={() => {
                              const banquetForEdit: BanquetFormState = {
                                ...
                                uploadWeddingBrochure: [],
                                uploadCancelledCheque: [],
                                resortPhotos: (banquet as any).uploadResortPhotos || [],
                                marriagePhotos: (banquet as any).uploadMarriagePhotos || [],
                                weddingBrochure: (banquet as any).uploadWeddingBrochure || [],
                                cancelledCheque: (banquet as any).uploadCancelledCheque || [],
                                resortPhotosPreview: [],
                                marriagePhotosPreview: [],
                                weddingBrochurePreview: [],
                                cancelledChequePreview: [],
