...
import { fetchBanquets, selectBanquets } from "@/lib/redux/features/banquetSlice";

// This should match the Omit type in your slice
type NewEnquiryState = Omit<BanquetEnquiry, "id" | "createdAt" | "updatedAt">;

const initialEnquiryState: NewEnquiryState = {
  name: "",
  email: "",
  phoneNumber: "",
  status: BanquetEnquiryStatus.NEW,
  authId: "",
  message: "",
};
...
export default function BanquetEnquiryPage() {
  ...
  const banquets = useAppSelector(selectBanquets);
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [selectedEnquiry, setSelectedEnquiry] = useState<BanquetEnquiry | null>(
    null
  );
  ...
  useEffect(() => {
    dispatch(fetchBanquetEnquiries());
  }, [dispatch]);
  ...
  const filteredEnquiries = enquiries.filter((enquiry: BanquetEnquiry) => {
  ...
  const getStatusColor = (status: BanquetEnquiry["status"]) => {
    switch (status) {
      case BanquetEnquiryStatus.NEW:
        return "bg-blue-100 text-blue-800";
      case BanquetEnquiryStatus.IN_PROGRESS:
        return "bg-yellow-100 text-yellow-800";
      case BanquetEnquiryStatus.COMPLETED:
  ...
  const handleAdd = () => {
    ...
    dispatch(createBanquetEnquiry(newEnquiry))
      .then(() => {
  ...
  const handleUpdate = () => {
    ...
    dispatch(updateBanquetEnquiry({ id: selectedEnquiry.id, data: updateData }))
      .unwrap()
      .then(() => {
  ...
  const getBanquetName = (authId: string) => {
    if (!banquets || (Array.isArray(banquets) && banquets.length === 0)) {
      return "Unknown Banquet";
    }
    const banquet = (banquets as any[]).find((b: any) => String(b?.id) == String(authId));
    return banquet?.venueName || "Unknown Banquet";
  };
      ...
      <section>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => {
              setNewEnquiry(prev => ({ ...prev, status: value as BanquetEnquiryStatus }));
            }}
          >
            <SelectTrigger>
            ...
            <SelectContent>
              <SelectItem value={BanquetEnquiryStatus.NEW}>New</SelectItem>
              <SelectItem value={BanquetEnquiryStatus.IN_PROGRESS}>In Progress</SelectItem>
              <SelectItem value={BanquetEnquiryStatus.COMPLETED}>Completed</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </section>
        ...
        <section>
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => {
                setSelectedEnquiry((prev: BanquetEnquiry | null) => 
                  prev ? { ...prev, status: value as BanquetEnquiryStatus } : null
                );
              }}
            >
              <SelectTrigger>
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage banquet enquiries and their status</p>
        </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading && enquiries.length === 0 ? (
              ...
              filteredEnquiries.map((enquiry: BanquetEnquiry) => (
                <motion.tr
                  key={enquiry.id}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
